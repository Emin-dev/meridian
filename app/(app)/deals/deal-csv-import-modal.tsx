...
import { bulkImportDeals } from "./actions";
import type { ImportSkippedRow } from "@/lib/csv";
import { parseDealsCsv, type ParsedDealRow } from "@/lib/csv";
...
export default function DealCsvImportModal({ hasDb }: Props) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const { toast } = useToast();
  const [isPending, startTransition] = useTransition();

  const [csvText, setCsvText] = useState("");
  const [parsed, setParsed] = useState<ParsedDealRow[]>([]);
  ...
  function handleImport() {
    ...
    startTransition(async () => {
      try {
        const result = await bulkImportDeals(parsed);
        if (result.error) {
      ...
      >
        {mode === "results" ? (
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 z-10 bg-[var(--surface-1)]">
                        <tr className="border-b border-[var(--line-1)] bg-[var(--surface-2)]">
                          ...
                          <th className="px-3 py-2 text-left font-medium uppercase tracking-wide text-[var(--ink-3)]">Deal</th>
          ...
          <>
            ...
            <div className="flex items-center justify-between border-b border-[var(--line-1)] px-6 py-4 shrink-0">
              <div>
                <h2 className="text-base font-semibold">Import Deals from CSV</h2>
                <p className="mt-0.5 text-xs text-[var(--ink-3)]">
                  Columns: <code className="rounded bg-[var(--surface-2)] px-1">title</code>,{" "}
                  <code className="rounded bg-[var(--surface-2)] px-1">value</code>,{" "}
                  <code className="rounded bg-[var(--surface-2)] px-1">stage</code>,{" "}
                  <code className="rounded bg-[var(--surface-2)] px-1">currency</code>,{" "}
                  <code className="rounded bg-[var(--surface-2)] px-1">contact_email</code>
                </p>
              </div>
              <button
                type="button"
                ...
                <>
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={"title,value,stage,currency,contact_email\nAcme renewal,5000,proposal,USD,jane@example.com"}
                  ...
                  {parseSkipped.length > 0 && (
                    <p className="text-xs text-[var(--warn)]">
                      {parseSkipped.length} row{parseSkipped.length !== 1 ? "s" : ""} will be skipped (missing title).
                    </p>
                  )}
                    ...
                    <div>
                      ...
                      <div className="overflow-x-auto rounded-lg border border-[var(--line-1)]">
                        <table className="w-full text-xs">
                          <thead>
                            <tr className="border-b border-[var(--line-1)] bg-[var(--surface-2)]">
                              {["Title", "Value", "Stage", "Currency", "Contact"].map((h) => (
                                <th
                                  key={h}
                          ...
                          <tbody>
                            {parsed.slice(0, PREVIEW_LIMIT).map((row, i) => (
                              ...
                              >
                                <td className="px-3 py-2 font-medium text-[var(--ink-1)]">{row.title || "—"}</td>
                                <td className="px-3 py-2 text-[var(--ink-2)]">{row.value || "—"}</td>
                                <td className="px-3 py-2 text-[var(--ink-2)]">{row.stage || "—"}</td>
                                <td className="px-3 py-2 text-[var(--ink-2)]">{row.currency || "—"}</td>
                                <td className="px-3 py-2 text-[var(--ink-2)]">{row.contactEmail || "—"}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
